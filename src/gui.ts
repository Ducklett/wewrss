...
const linkifyMedia = (root: HTMLElement) => {
  ...
  for (let med of media) {
    ...
    med.addEventListener('click', () => {
      const source =
        (med as HTMLImageElement).src || med.querySelector('source').src
...
const inlineChannelRenderer = (
  head: RssHeader,
  onClick: () => any,
  ...children: HTMLElement[]
) =>
  Div(
    { class: 'channel-info-inline' },
...
const dateAndChannelInfo = (
  head: RssHeader,
  date: Date,
  onChannelNameClick: () => any
) =>
  head
    ? inlineChannelRenderer(head, onChannelNameClick, DateTime(date))
    : DateTime(date)

/**inserts <br>'s for every newline */
function brIfy (html: string) {
  return html.replace(/\n/g, '\n<br>')
}
...
const feedGuis: FeedTypeGuis = {
    ...
    render: (a: ArticleGuiData) =>
      Article(
        ...
        linkifyMedia(
          Main({
            innerHTML:
              brIfy(a.description) || A({ href: a.link }, P({ text: a.title }))
          })
        )
      )
  },
  ...
  [FeedType.Article]: {
    ...
    render: (a: ArticleGuiData) =>
      Article(
        {},
        A({ href: a.link, target: '_blank' }, H2({ text: a.title })),
        dateAndChannelInfo(
          a.channelHeader,
          a.date,
          loadIndividualChannel(a.channelHeader.name)
...
const makeConfig = (corsProxy: string, channels: Channel[]) => ({
  corsProxy,
  channels
})
...
export default async ({
    ...
    let articles: ArticleGuiData[] = articleMap
      ...
      .map(
        ({ guid, title, link, date, description, thumbnail, channelName }) => ({
          ...
          thumbnail,
          channelHeader:
            channel.kind == 'aggregate' && data.headerMap.get(channelName)
        })
      )
  ...
  const renderLastUpdated = () => {
    lastUpdated.textContent = `Last updated: ${localStorage.getItem(
      'lastUpdated'
    ) || ''}`
  }
  ...
  const showData = async (forceUpdate = false, page = 0) => {
    ...
    if (oldData.articleMap.size > 0 && !forceUpdate) {
      ...
      ;[data, flatArticleMap] = await updateFeeds(
        channels,
        corsProxy,
        countUpdated
      )
  ...
  prevBtn.addEventListener('click', () =>
    showFeed(lastChannel, currentPage - 1)
  )
